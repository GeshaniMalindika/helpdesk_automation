...
class Member
{
    ...
    {
        $query = 'SELECT * FROM asset_user where username = ?';
        $paramType = 's';
        $paramValue = array(
            $username
        );
    ...
    public function handleForgot()
    {
        if (! empty($_POST["username"])) {
            ...
            if (! empty($memberRecord)) {
                $passwordReset->insert($memberRecord[0]['emp_id'], $token);
    ...
    public function updatePassword($emp_id, $password)
    {
        ...
        $query = 'UPDATE asset_user SET password = ? WHERE emp_id = ?';
        $paramType = 'si';
        $paramValue = array(
            $hashedPassword,
            $emp_id
